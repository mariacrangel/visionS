...
ApplicationWindow {
    ...
    property var latitud
    property var longitud

    Connections {
      target: receiver
      onSendToQmlLongi: {
        console.log("Longitude catch from IP: " + longi)
          longitud = longi;

      }
      onSendToQmlLat: {
        console.log("Latitude catch from IP: " + lat)
          latitud = lat
      }
    ...
    //defaults
    //! [routecoordinate]
     property variant fromCoordinate: QtPositioning.coordinate(parseFloat(longitud), parseFloat(latitud))
     property variant toCoordinate: QtPositioning.coordinate( parseFloat(longitud), parseFloat(latitud))
    //! [routecoordinate]
    ...
    property variant locationVS: QtPositioning.coordinate( parseFloat(longitud), parseFloat(latitud))
    ...
    function createMap(provider)
    {
        ...
        if (zoomLevel != null) {
            ...
            map.zoomLevel = (map.maximumZoomLevel - map.minimumZoomLevel)/5
        }
    ...
    Component {
        ...
        MapQuickItem {
            ...
            Component.onCompleted: {
                ...
                coordinate = QtPositioning.coordinate( Qt.point(markerMouseArea.mouseX, markerMouseArea.mouseY))
                update()
            }
            }
